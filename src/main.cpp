...
#define X_ANALOG_PIN A3
#define Y_ANALOG_PIN A2
#define Z_ANALOG_PIN A1
...
volatile uint32_t sample_us = 4000;        // 250 Hz default
...
//         int16_t rz = (raw[5] << 8) | raw[4];
        ...
        int16_t rx = analogRead(X_ANALOG_PIN);
        int16_t ry = analogRead(Y_ANALOG_PIN);
        int16_t rz = analogRead(Z_ANALOG_PIN);
